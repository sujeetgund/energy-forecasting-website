...
export function LinearProcessFlow({ children }: LinearProcessFlowProps) {
  const steps = React.Children.toArray(children)
    .filter((child): child is React.ReactElement => React.isValidElement(child) && child.type === 'h3')
    .map((step, index) => {
      const content = React.Children.toArray(children).find(
        (child, i) => i > React.Children.toArray(children).indexOf(step) && React.isValidElement(child) && child.type !== 'h3'
      )
